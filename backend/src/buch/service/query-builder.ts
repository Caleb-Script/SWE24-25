    ...
    build({ titel, javascript, typescript, isbn, ...props }: Suchkriterien) {
        ...
        if (javascript !== undefined) {
            if (javascript) {
                ...
                queryBuilder = useWhere
                    ? queryBuilder.where(
                          `${this.#buchAlias}.schlagwoerter not like '%JAVASCRIPT%'`,
                      )
                    : queryBuilder.andWhere(
                          `${this.#buchAlias}.schlagwoerter not like '%JAVASCRIPT%'`,
                      );
        ...
        if (typescript !== undefined) {
            if (typescript) {
                ...
                queryBuilder = useWhere
                    ? queryBuilder.where(
                          `${this.#buchAlias}.schlagwoerter not like '%TYPESCRIPT%'`,
                      )
                    : queryBuilder.andWhere(
                          `${this.#buchAlias}.schlagwoerter not like '%TYPESCRIPT%'`,
                      );
        ...
        if (javascript !== undefined && typescript !== undefined) {
            // eslint-disable-next-line unicorn/no-lonely-if
            if (!javascript && !typescript) {
                queryBuilder = queryBuilder.orWhere(
                    `${this.#buchAlias}.schlagwoerter IS NULL`,
                );
            }
        }
